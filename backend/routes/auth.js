 ...
 * 3. /request-otp  — WhatsApp a 6-digit code (first-time setup / forgot PIN)
 * 4. /verify-otp   — code -> short-lived pin-reset token
 * 5. /set-pin      — store the new PIN. REQUIRES the reset token from step 4,
 *                    so knowing someone's phone number alone can never take
 *                    over their account.
...
/* ---------------- WhatsApp OTP (guards PIN create/reset) ----------------
 * Codes live in the otp_codes table: bcrypt-hashed, single-use, 5-minute
 * expiry, 5 wrong guesses max. Sending is throttled per phone (60s cooldown,
 * 8/day) so the endpoint can't be used to spam an employee's WhatsApp. */
...
// ===== Request a code: POST /auth/request-otp { phone } =====
router.post('/request-otp', otpSendLimiter, async (req, res) => {
  try {
    ...
    const user = await prisma.user.findFirst({
      where: { phone, isActive: true }, select: { id: true },
    });
    if (!user)
    ...
    const gate = await prisma.$transaction(async (tx) => {
      ...
      const latest = await tx.otpCode.findFirst({ where: { phone }, orderBy: { id: 'desc' } });
      if (latest) {
      ...
      if ((await tx.otpCode.count({ where: { phone } })) >= OTP_MAX_PER_DAY)
    ...
    if (!sent.ok) {
      ...
      return res.status(502).json({ message: 'Could not send the WhatsApp code. Please try again.' });
    }
    res.json({ ok: true, resendIn: OTP_RESEND_COOLDOWN_S, ttlMin: OTP_TTL_MIN });
  } catch (e) { console.error(e); res.status(500).json({ message: 'Server error' }); }
});
...
router.post('/verify-otp', async (req, res) => {
  try {
    ...
    if (phone.length !== 10 || !/^\d{6}$/.test(otp))
      return res.status(400).json({ message: 'Phone and 6-digit code required' });

    const row = await prisma.otpCode.findFirst({ where: { phone }, orderBy: { id: 'desc' } });
    if (!row)
    ...
    const user = await prisma.user.findFirst({ where: { phone, isActive: true }, select: { pinHash: true } });
    if (!user) return res.status(404).json({ message: 'Phone not registered. Contact your admin.' });
    const resetToken = jwt.sign(
      { phone, type: 'pin-reset', bind: pinBind(user.pinHash) },
...
router.post('/set-pin', async (req, res) => {
  try {
    ...
    // OTP gate: no valid reset token for THIS phone -> no PIN change. Ever.
    let claim = null;
    try { claim = jwt.verify(String(req.body?.resetToken || ''), process.env.JWT_SECRET); } catch {}
    if (!claim || claim.type !== 'pin-reset' || claim.phone !== phone)
      return res.status(401).json({
